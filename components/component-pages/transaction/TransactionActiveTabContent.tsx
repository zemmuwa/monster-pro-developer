...
function TransactionActiveTabContent() {
	...
	const transactionGetAllWParams = async (params?: { isNext?: boolean }) => {
		...
		filters.push(['AND'])
		filters.push([
			['booking.status_dev', 'IS', null],
			['OR'],
			['booking.status_dev', true],
		])
		filters.push(['AND'])
		filters.push([
			['booking.status_buyer', 'IS', null],
			['OR'],
			['booking.status_buyer', true],
		])
		if (filter?.search) {
			filters.push(['AND'])
			filters.push([
		...
		<>
			<Grid mb={2} container spacing={2} sx={{ px: 4 }}>
				<Grid
					...
					alignItems="center"
				>
					<Typography variant="h4">Transaksi Aktif</Typography>
				</Grid>
				<Grid
					item
					xs={12}
...
export default TransactionActiveTabContent
