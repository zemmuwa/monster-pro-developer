...
interface PropsFinishedTransactionTab {
	bookingId?: string
	showFile?: boolean
	withReceipt?: boolean
}

function FinishedTransactionTab({
	bookingId,
	showFile,
	withReceipt,
}: PropsFinishedTransactionTab) {
	...
	const trxGetAllWParams = async (params?: { isNext?: boolean }) => {
		...
		const filter: any[] = [
			...
			['AND'],
			[['redeem_status', 3], ['OR'], ['redeem_status', 4]],
		]
		if (bookingId) {
			filter.push(['AND'])
...
export default FinishedTransactionTab
